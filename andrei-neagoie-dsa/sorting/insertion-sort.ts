...
export function insertionSort(nums: number[]): void {
  ...
  for (let i = 0; i < nums.length; i++) {
    // This loop will run from next element from i, and starts the loop to previous element till it reaches 0 + 1, because it checks for j - 1 element in the array. It checks if the element is smaller than the previous element is just swap it till it reaches the start of the array.
